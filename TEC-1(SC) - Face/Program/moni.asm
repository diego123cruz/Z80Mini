...
;		tasm -80 -fff -c MONI.ASM moni.bin
; 
; GRAVAÇÃO:
;	minipro -p AT28C64B -w moni.bin -s	
;
;
; SERIAL MAC:
; 	SerialTools(Terminal comandos) e WCHSerialPort(Enviar intel hex)
;
;
;
;
...
; VERSION 1.6  09/2021  CONDITIONAL ASSEMBLY ALLOWS FOR TARGET TO BE TEC-1F OR SOUTHERN CROSS
;
; VERSION 1.7  09/2022  AJUSTE PARA Z80Mini (32k ROM, 32k RAM, 4Mhz), Ajustes memory map and Ports IO
;						Onboad step hardware (Port 60H)
;
;THE TARGET IS THE SOUTHERN CROSS
#DEFINE TEC-1F
;
;#IFDEF TEC-1F
...
; SYSTEM VARIABLES
;-----------------
RAMSRT:	.EQU	8000H	;START OF USER RAM
RAMEND:	.EQU	0FBFFH	;END OF USER RAM
...
PORTC0:      .EQU 0C0H             ;ST3 (8X8), STROBE (RELAY BOARD) DATLATCH (DAT BOARD)
PORT4:       .EQU 04H             ;ST4 (8X8), LCD 'E' (DAT BOARD)
PORT5:       .EQU 05H
PORT6:       .EQU 06H
PORT60:      .EQU 60H             ;ENABLE/DISABLE SINGLE STEPPER (IF INSTALLED)
#ELSE
...
ISTACK:  .EQU 0FFAEH   ;SAVE THE INITIAL STACK POINTER    
ADDR:    .EQU 0FFB0H   ;THE ADDRESS  
DATA:    .EQU 0FFB2H   ;THE DATA
;
SYSFLG:	.EQU	0FFB3H	;SYSTEM FLAGS  BIT 0=KEYBOARD FLAG
FUNTBL:	.EQU	0FFB4H	;FN TABLE ADDRESS
;
; DALLAS SMARTWATCH REGISTERS
;
CALMDE:	.EQU	0FFB6H	;CALENDAR MODE
SWREG0:	.EQU	0FFB8H	;10THS, 100THS
SWREG1:	.EQU	0FFB9H	;SECONDS
SWREG2:	.EQU	0FFBAH	;MINUTES
SWREG3:	.EQU	0FFBBH	;HOURS
SWREG4:	.EQU	0FFBCH	;DAY
SWREG5:	.EQU	0FFBDH	;DATE
SWREG6:	.EQU	0FFBEH	;MONTH
SWREG7:	.EQU	0FFBFH	;YEAR
;
BAUD:	.EQU	0FFC0H	;BAUD RATE
KEYTIM:	.EQU	0FFC2H	;BEEP DELAY
SPTEMP:	.EQU	0FFC4H	;TEMP SYSTEM CALL SP
;
; BLOCK FUNCTIONS
;
COUNT:	.EQU	0FFC6H	;NUMBER OF BYTES TO MOVE
BLKSRT:	.EQU	0FFC8H	;BLOCK START ADDRESS
BLKEND:	.EQU	0FFCAH	;BLOCK END ADDRESS
BLKDST:	.EQU	0FFCCH	;DESTINATION ADDRESS
;
FUNJMP:	.EQU	0FFCEH	;FN FN KEY JUMP ADDRESS
;
; DISPLAY SCAN REGISTERS
;
DISBUF:	.EQU	0FFD0H	;DISPLAY BUFFER
ONTIM:	.EQU	0FFD6H	;DISPLAY SCAN ON TIME
OFTIM:	.EQU	0FFD7H	;DISPLAY SCAN OFF TIME
;
; MONITOR VARIABLES
;
MODE:	.EQU	0FFD8H	;DISPLAY MODE
ADRESS:	.EQU	0FFDAH	;USER ADDRESS
KEYDEL:	.EQU	0FFDCH	;AUTO INCREMENT DELAY
;
; TEMPORARY REGISTER STORAGE
;
REGPNT:	.EQU	0FFDEH	;REGISTER POINTER
TMPPC:	.EQU	0FFE0H	;PROGRAM COUNTER
TMPAF:	.EQU	0FFE2H	;ACCUMULATOR,FLAG
TMPBC:	.EQU	0FFE4H	;BC REGISTER PAIR
TMPDE:	.EQU	0FFE6H	;DE REGISTER PAIR
TMPHL:	.EQU	0FFE8H	;HL REGISTER PAIR
TMPIX:	.EQU	0FFEAH	;INDEX REGISTER X
TMPIY:	.EQU	0FFECH	;INDEX REGISTER Y
TMPSP:	.EQU	0FFEEH	;STACK POINTER
;
; RESTART JUMP TABLE AND HARWARE TEST
;
RST08:	.EQU	0FFF0H	;RESTART 08H JUMP
RST10:	.EQU	0FFF2H	;RESTART 10H JUMP
RST18:	.EQU	0FFF4H	;RESTART 18H JUMP
RST20:	.EQU	0FFF6H	;RESTART 20H JUMP
RST28:	.EQU	0FFF8H	;RESTART 28H JUMP
RST38:	.EQU	0FFFAH	;INT INTERRUPT JUMP
RST66:	.EQU	0FFFCH	;NMI INTERRUPT JUMP
RAMSUM:	.EQU	0FFFEH	;USER RAM CHECKSUM
DALLAS:	.EQU	0FFFFH	;RAM TEST LOCATION
;----------------
; RESTART VECTORS
;----------------
;
...
;
; SET UP function table 2 if pressed shift
;
	IN  A, (KEYBUF)
	BIT 5, A
	JP NZ, TBL2_SKP
...
#IFDEF TEC-1F    
	...
	BIT	6,A		        ;DA LOW? MUST BE SOFTWARE SCANNING
	JR	NZ,MAIN    
#ELSE    
	IN  A,(KEYBUF)      ;READ KEY BUFFER
...
;
#IFDEF TEC-1F
SETDP:       SET 7,(HL)          ;SET BIT 4 FOR DP ON TEC-1F DISPLAY
#ELSE
...
#IFDEF TEC-1F
;    LD  HL,4764H  ;FN
    LD HL,7154H       ;USE THE GO KEY IN THE TEC
#ELSE
	LD	HL,754H  ;FN
#ENDIF
...
CANCL1:	CALL	UPDATE	;UPDATE DISPLAY BUFFER
	...
	JP	MAIN2
;
; FUNCTION MENU KEY TABLE2
;
FUNLST2:	.DB	10H
	.DB	00H,01H,02H,03H,04H,05H,06H,07H
	.DB	08H,09H,0AH,0BH,0CH,0DH,0EH,0FH
	.DW	ON_LED, OFF_LED, ON_LED, OFF_LED
	.DW	ON_LED, OFF_LED, ON_LED, OFF_LED
	.DW	ON_LED, OFF_LED, ON_LED, OFF_LED
	.DW	ON_LED, OFF_LED, ON_LED, OFF_LED
...
TRACE:	CALL	BEEP
#IFDEF TEC-1F
        OUT	(PORT60),A	    ;TOGGLE HARDWARE LATCH
        LD HL,0078H         ;SHOW T
#ELSE
...
SKATE3:	DJNZ	SKATE3	;OFF TIME DELAY
	...
	RET
;----------------------------
; FUNCTION F - CLOCK CALENDAR -> SIMON
;----------------------------
...
DPLAY3: CALL  INCH
       JR    C,DPLAY3   
       CP    20H        ;SPACE DISPLAYS THE NEXT LINE
       JR    Z,DPLAY2
       CP    ESC         ;ESC EXITS
       JR    NZ,DPLAY3     
       RET   
...
;
CRLF:        .DB     CR,LF,00H
SIGNON:      .DB     "SCM 1.5 SERIAL MONITOR",CR,LF,
			 .DB	 "ACCEPTS ONLY UPPER CASE"
			 .DB	 " - H FOR COMMAND LIST",
			 .DB	 CR,LF,00H
REGS:        .DB     CR,LF,
			 .DB 	 "PC   AF   BC   DE   "
			 .DB	 "HL   IX   IY   SP   "
			.DB		 "SZ-H-VNC",
			 .DB	 CR,LF,00H
TOGGLE:      .DB     CR,LF,"SINGLE STEP TOGGLE",CR,LF,00H
TXFILE:      .DB     CR,LF,"SEND INTEL HEX FILE...",00H
FILEOK:      .DB     CR,LF,"FILE RECEIVED OK",CR,LF,00H
CSUMERR:     .DB     CR,LF,"CHECKSUM ERROR",CR,LF,00H
BYE:         .DB     CR,LF,"BYE...",CR,LF,00H

COMANDS:    .DB     CR,LF,"D AAAA    "
			.DB		"DISPLAY DATA AT ADDRESS AAAA"
            .DB     CR,LF,"T         "
			.DB		"TOGGLE SINGLE STEPPER"           
            .DB     CR,LF,"M AAAA    "
			.DB		"MODIFY ADDRESS AAAA",00H
            
COMANDS1:    .DB     CR,LF,"G AAAA    GO FROM AAAA"     
            .DB     CR,LF,"I         DOWNLOAD INTEL HEX"
            .DB     CR,LF,"X         EXIT"
            .DB     CR,LF,"H         "
			.DB 	"THIS HELP MESSAGE",CR,LF,00H
;
